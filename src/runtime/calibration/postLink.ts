...
export function findFirstPostPermalinkLink(root: QueryRoot = document): HTMLAnchorElement | null {
  ...
  const selectors = [
    ...LINK_SELECTORS.map((selector) => `${ARTICLE_SELECTOR_PREFIX}${selector}`),
    ...LINK_SELECTORS
  ];
  ...
  for (const selector of selectors) {
    ...
    for (const link of candidates) {
      ...
      if (!href) continue;
      if (!isLikelyPostPermalink(href, baseHref)) continue;
      return link;
    }
  }
