...
export function extractProfileUsername(rawUrl: string | null | undefined): string {
  ...
  if (!first || RESERVED_PROFILE_SEGMENTS.has(lowered)) return "";
...
export function extractProfileIdFromArtifact(artifact: GraphqlArtifactV1 | null | undefined): string {
  if (!artifact?.entries) return "";
  const entry = artifact.entries[TIMELINE_QUERY_NAME];
  if (!entry || typeof entry.variables !== "object" || !entry.variables) return "";
...
export function resolveCollectionFolderName(folderNames: string[] | undefined, collectionId: string | undefined): string {
...
export function buildCollectionRelativeFilename(
  folderName: string,
