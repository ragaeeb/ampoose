...
function pickJsonPayload(payloads: unknown[]): unknown {
  for (const payload of payloads) {
    ...
    if ("data" in record || "errors" in record) return payload;
  }
  return payloads[0];
}
...
function resolveEndpointCandidates(inputEndpoint?: string): string[] {
  if (inputEndpoint) return [inputEndpoint];
...
function getAmbientRequestParams(): Record<string, string> {
  ...
  if (fbDtsg) {
    ...
    if (jazoest) params.jazoest = jazoest;
  }
  ...
  try {
    if (typeof window !== "undefined") {
      ...
      if (typeof spinB === "string" && spinB) params.__spin_b = spinB;
...
async function parseGraphqlResponseBody(
  ...
  try {
    ...
    if (payloads.length > 0) {
      if (responseMode === "all") return payloads;
...
export function createGraphqlClient(deps: GraphqlClientDeps) {
  ...
  async function requestEndpoint(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `GraphQL request failed: ${response.status}${preview ? ` preview="${preview}"` : ""}`
      );
    }
  ...
  async function request<T = unknown>(input: GraphqlRequestInput): Promise<T> {
    const artifact = await deps.loadArtifact();
    ...
    const pushVariant = (params: Record<string, string>) => {
      ...
      paramsVariants.push(clean);
    };

    pushVariant({ ...ambientParams, ...capturedParams });
    pushVariant({ ...ambientParams });
    pushVariant({});
    ...
    for (const endpoint of endpoints) {
      for (const params of paramsVariants) {
        ...
        body.set("doc_id", entry.docId);

        try {
          return (await requestEndpoint(endpoint, input.queryName, body, input.responseMode ?? "single")) as T;
        } catch (error) {
          const details = String(error instanceof Error ? error.message : error);
          errors.push(`endpoint=${endpoint} params=${Object.keys(params).length} error=${details}`);
        }
      }
    }
